...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".img:not([data-origin])").forEach((img, index) => {
    img.setAttribute("data-origin", index % 2 === 0 ? "left" : "right"); // ✅ Fixed extra space
  });
      ...
      ScrollTrigger.create({
        ...
        onUpdate: function (self) {
          if (self.isActive) {
            ...
            const isAbove = self.scroll() < self.start; // ✅ Fixed incorrect function call
            if (isAbove) {
              gsap.set(rowImages, { scale: 1, force3D: true });
            }
          }
  ...
  window.addEventListener("resize", () => {
    ScrollTrigger.refresh(); // ✅ Removed invalid boolean argument
  });
});
